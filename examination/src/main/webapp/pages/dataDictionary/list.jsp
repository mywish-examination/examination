            ...
            <div class="ibox ">
                ...
                <div class="ibox-content">
                    ...
                    <div class="jqGrid_wrapper">
                        <table id="dataDictionaryList"></table>
                        <div id="pager"></div>
                    </div>
...
<script>
    $(document).ready(function () {
        ...
        $("#dataDictionaryList").jqGrid({
            url: "${basePath}web/dataDictionary/listPage",
            ExpandColumn: 'name',
            ExpandColClick: true,
            height: 520,
            autowidth: true,
            ...
            mtype: "POST",
            colNames: ['', '字典名称', '字典编码' , '字典值', '字典数值', '操作'],
            colModel: [
                {name: 'id', index: 'id', hidden: true},
                {name: 'dictName', index: 'dictName', width: '25%', sortable: false},
                {name: 'dictCode', index: 'dictCode', width: '25%', sortable: false},
                {name: 'dictValue', index: 'dictValue', width: '25%', sortable: false},
                {name: 'dictNum', index: 'dictNum', width: '10%', sortable: false},
            ...
            gridComplete: function() {
                var ids = jQuery("#dataDictionaryList").jqGrid('getDataIDs');
                for(var i=0;i < ids.length;i++){
                    ...
                    content += "</a>";
                    jQuery("#dataDictionaryList").jqGrid('setRowData',ids[i],{act:"<div class='jqgridContainer'>" + content + "</div>"});
                }
            },
            loadComplete: function(){
                ...
                $(".shortcut_delete").click(function(){
                    ...
                    var url = "${basePath}web/dataDictionary/delete?id=" + rowid;
                        ...
                        $.ajax({
                            ...
                            success:function(jsonData){
                                if(jsonData.status == 'success') {
                                    ...
                                    $("#dataDictionaryList").trigger("reloadGrid");
                                }
                            }
                        });
                    }, function(){
                ...
                $(".shortcut_modify").click(function() {
                    var rowid = $(this).attr("id");
                    window.location.href = "${basePath}web/dataDictionary/detail?id=" + rowid;
                });
            }
        });
        ...
        $(window).bind('resize', function () {
            ...
            $('#dataDictionaryList').setGridWidth(width);
        });
        ...
        var $content = $("<a></a>").attr("href","javascript:void(0)")
            ...
            .append("创建");
        $("#t_dataDictionaryList").append("&nbsp;&nbsp;").append($("<span></span>").attr("class","jqgridContainer").append($content));
        $("#create","#t_dataDictionaryList").click(function(){
            window.location.href = "${basePath}web/dataDictionary/detail";
        });
        ...
        $content = $("<a></a>").attr("href","javascript:void(0);")
            ...
            .append("删除");
        $("#t_dataDictionaryList").append("&nbsp;&nbsp;").append($("<span></span>").attr("class","jqgridContainer").append($content));
        $("#deleteBatch","#t_dataDictionaryList").click(function(){
            //获取多选到的id集合
            var ids = $("#dataDictionaryList").jqGrid("getGridParam", "selarrrow");
            if(ids == null || ids == "") return;

            var prompt = "确定要删除所选择的记录吗？";
            var url = "${basePath}web/dataDictionary/deleteBatch?ids=" + ids;
            index = top.layer.confirm(prompt, {
                btn: ["确认", "取消"] //按钮
            }, function(){
                $.ajax({
        ...
        $content = $("<a></a>").attr("href","javascript:void(0);")
            ...
            .append("清空全部");
        $("#t_dataDictionaryList").append("&nbsp;&nbsp;").append($("<span></span>").attr("class","jqgridContainer").append($content));
        $("#deleteAll","#t_dataDictionaryList").click(function(){
            var prompt = "确定要删除所选择的记录吗？";
            var url = "${basePath}web/dataDictionary/deleteAll";
            index = top.layer.confirm(prompt, {
                btn: ["确认", "取消"] //按钮
            }, function(){
                $.ajax({
        ...
        $content = $("<a></a>").attr("href","javascript:void(0);")
            ...
            .append("导入");
        $("#t_dataDictionaryList").append("&nbsp;&nbsp;").append($("<span></span>").attr("class","jqgridContainer").append($content));
        $("#import","#t_dataDictionaryList").click(function(){
            showUploadDialog();
        });
    });
    ...
    function showUploadDialog(){
        picUpload({
            server: '${basePath}web/dataDictionary/uploadFileExcel;jsessionid=<%=session.getId()%>?param=pic',
            accept: {
                title: 'Images',//文字描述
