	...
	$.Valid.prototype = {
		...
		init: function() {
			...
			for(var i=0;i<classNames.length;i++){
				...
				var element = $("<span>")
				.attr( "id", $("." + classNames[i]).attr('id') + "-error" )
				.addClass( "help-block m-b-none" );
				$( element ).insertAfter($("." + classNames[i]));
			}
			//设置验证控件元素触发事件
			if (settings.event){
				for(var i=0;i<classNames.length;i++){
				...
				$.extend(settings.ajax,{success:function(jsonData){
								...
								for (var r=0;r<jsonData.result.length;r++){
									...
									if (name == field){
						                ...
						                $('html,body').animate({
						                	...
						                	$tempObj.closest('.form-group').addClass('has-error');
						                	$tempObj.next().html(msg);
						                	if(settings.tabs.length>0){
						                		var tavsLength=settings.tabs.length;
						                		for(var i=0;i<tavsLength;i++){
						                			if($("#"+settings.tabs[i]).has("#"+id).length>0){
						                				$("#tab_"+(i+1)).addClass("active").siblings().removeClass("active");
						                				$("#"+settings.tabs[i]).css({"visibility": "visible"}).siblings().css({"visibility": "hidden"});
						                				return;
						                			}
						                		}
						                	}
											...
											if (tempName == field){
								                ...
								                $('html,body').animate({
								                	...
								                	if(settings.tabs.length>0){
								                		var tavsLength=settings.tabs.length;
								                		for(var i=0;i<tavsLength;i++){
								                			if($("#"+settings.tabs[i]).has("#"+tempId).length>0){
								                				$("#tab_"+(i+1)).addClass("active").siblings().removeClass("active");
								                				$("#"+settings.tabs[i]).css({"visibility": "visible"}).siblings().css({"visibility": "hidden"});
								                				return;
								                			}
					...
					if (validStatus && obj == undefined){
						if(typeof settings.validStatus == 'function'){
							if(validStatus){
								if(settings.isDisabled  && settings.buttonObj != undefined && settings.buttonObj != null){
                            		settings.buttonObj.attr('disabled',true);
                            	}
							}
